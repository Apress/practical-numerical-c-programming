...
#include<stdio.h>
	...
	/* Each record contains data for one patient. */
	/* The data is the patients ID,name and blood pressure */
	struct Patient {
	int PatientID;
	char name[13];
	int BloodPressure;
...
int main()
{
	...
	struct Patient s1;
	struct Patient s2;
	/* Preset the data for each patient */
	...
	for (i = 0;i < 17;i++)
	{
		numread = fread(&s2, sizeof(s2), 1, fp);/* read into numread */
		
		if (numread == 1)
		{
			/* print data for one patient */
